...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelector('#addTaskModal form').addEventListener('submit', function (event) {
      ...
      // Reset the form
      this.reset();
      bootstrap.Modal.getInstance(document.querySelector('#addTaskModal')).hide();
    });
    ...
    taskTable.addEventListener('click', function (event) {
      if (event.target.classList.contains('btn-warning')) {
        ...
        document.getElementById('updateTaskId').value = row.children[0].textContent;
        document.getElementById('updateDescription').value = row.children[1].textContent;
        document.getElementById('updateDueDate').value = row.children[2].textContent;
        document.getElementById('updateAssignedTo').value = row.children[3].textContent;
        document.getElementById('updateCustomerId').value = row.children[4].textContent;
      }
    });
  
    // Update Task Form Submission
    document.querySelector('#updateTaskModal form').addEventListener('submit', function (event) {
      event.preventDefault();
      ...
      bootstrap.Modal.getInstance(document.querySelector('#updateTaskModal')).hide();
    });
